...
<?php
						$areaQuery = mysqli_query($con,"SELECT area FROM area ORDER BY number asc") or die(mysqli_error($con));		 
						$ignore = false;
						foreach($areaQuery as $area_row)
						{
							$area = $area_row['area'];
?>							
						
							<tr align="center" class="ignore">
							<th colspan="11"><?php echo $area;?></th>
							</tr>
							<tr align="center" style="font-weight:bold" <?php if($ignore == true) echo 'class="ignore"';?>>
...
<?php						$arListQuery =  mysqli_query($con,"SELECT ar_name,mobile FROM ar_details WHERE  area = '$area' ") or die(mysqli_error($con));		 
							$ignore = true;
							$arList = array();
							$mobileMap = array();
							foreach($arListQuery as $ar)
							{
								$arList[] = trim($ar['ar_name']);
								$mobileMap[$ar['ar_name']] = $ar['mobile'];
							}			
							$ar_detail = mysqli_query($con,"SELECT ar_name, target, payment_perc,rate FROM ar_calculation WHERE  month = '$month' AND Year='$year' AND target > 0 AND  ar_name IN('".implode("','",$arList)."') ORDER BY target desc") or die(mysqli_error($con));		 
...
?>
								<tr align="center">
								<td><?php echo $ar;?></b></td>
								<td><?php if(isset($mobileMap[$ar])) echo $mobileMap[$ar];?></b></td>
